            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {loading ? (
                    ...
                    levels.slice(0, 4).map((level, idx) => {
                        ...
                        return (
                            <div key={level.id} className={`bg-[#0B0B1E] border p-6 rounded-[32px] shadow-xl relative overflow-hidden transition-all duration-700 flex flex-col justify-between h-full group ${isCurrentLevel ? 'border-[#3DD6C8] scale-[1.02] shadow-[0_20px_60px_rgba(61,214,200,0.1)]' : 'border-white/5 hover:border-white/10'} ${isLocked ? 'opacity-40 grayscale-[0.5]' : ''}`}>
                                ...
                                <div className="space-y-6 relative z-10">
                                    <div className="flex justify-between items-start">
                                        <div className={`w-12 h-12 rounded-[20px] bg-white/5 border border-white/10 flex items-center justify-center group-hover:scale-110 transition-transform`}>
                                            <Icon size={24} className={isCurrentLevel ? 'text-[#3DD6C8]' : 'text-white/60'} />
                                        </div>
                                        <div className="flex flex-col items-end gap-2">
                                            {isCurrentLevel && (
                                                <div className="px-2 py-0.5 bg-[#3DD6C8]/10 border border-[#3DD6C8]/30 rounded-full flex items-center gap-1.5">
                                                    <div className="w-1 h-1 rounded-full bg-[#3DD6C8] animate-pulse" />
                                                    <span className="text-[7px] font-black text-[#3DD6C8] uppercase tracking-widest">Active</span>
                                                </div>
                                            )}
                                    ...
                                    <div>
                                        <h3 className="text-2xl font-black text-white italic uppercase tracking-tighter leading-none mb-1">{level.name || `ELITE LV.${level.id}`}</h3>
                                        <p className="text-[10px] font-black text-white/30 uppercase tracking-[0.3em] font-mono">Cost: <span className="text-white">${level.price.toLocaleString()}</span></p>
                                    </div>
                                    ...
                                    <div className="space-y-2 pt-4 border-t border-white/5">
                                        {[
                                            ...
                                            <div key={i} className="flex justify-between items-center">
                                                <span className="text-[7px] font-black text-white/20 uppercase tracking-[0.3em] flex items-center gap-1.5">
                                                    <item.icon size={10} className={item.color} /> {item.label}
                                                </span>
                                                <span className="text-[9px] font-black text-white uppercase tracking-widest">{item.value}</span>
                                            </div>
                                        ))}
                                    </div>
                                </div>
                                ...
                                <div className="mt-6 relative z-10">
                                    {isCurrentLevel ? (
                                        <div className="w-full py-3 bg-[#3DD6C8]/10 border border-[#3DD6C8]/20 rounded-[16px] text-[9px] font-black text-[#3DD6C8] uppercase tracking-[0.3em] text-center">
                                            SECURE NODE ACTIVE
                                        </div>
                                    ) : isLocked ? (
                                        <button className="w-full py-3 bg-white/5 border border-white/10 rounded-[16px] text-[9px] font-black text-white/40 uppercase tracking-[0.3em] hover:bg-white/10 hover:text-white transition-all">
                                            UPGRADE REQUIRED
                                        </button>
                                    ) : (
                                        <button className="w-full py-3 bg-emerald-500/10 border border-emerald-500/20 rounded-[16px] text-[9px] font-black text-emerald-400 uppercase tracking-[0.3em]">
                                            TIER DEPLETED
                                        </button>
                                    )}
                                </div>
