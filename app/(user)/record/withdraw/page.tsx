...
import { ArrowLeft, ArrowUpFromLine, Loader2, Calendar, MessageCircle, Share2, X, Copy, Check, ShieldAlert } from 'lucide-react';
...
const TransactionDetailModal = ({ tx, onClose }: { tx: Transaction; onClose: () => void }) => {
    ...
    const destinationAddress = tx.address || tx.wallet_address || 'Unspecified';
    ...
    const handleCopyVerification = () => {
        const msg = `[WITHDRAWAL VERIFICATION REQUEST]
...
Please verify and disburse my withdrawal.`;
        navigator.clipboard.writeText(msg);
        toast.success('Withdrawal details copied for Customer Support!');
    };
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-fade-in">
            <div className="bg-[#0B0B1E] border border-white/10 w-full max-w-sm rounded-[32px] overflow-hidden shadow-2xl animate-scale-in">
                <div className="p-6 border-b border-white/5 flex items-center justify-between">
                    <div>
                        <h3 className="font-black text-white uppercase tracking-widest text-sm">Disbursement Voucher</h3>
                        <p className="text-[9px] font-mono text-rose-400 uppercase font-bold mt-0.5">#{cleanTxId}</p>
                    </div>
                ...
                <div className="p-6 md:p-8 space-y-5">
                    <div className="text-center space-y-2">
                        <div className="w-16 h-16 rounded-2xl bg-rose-500/20 flex items-center justify-center mx-auto mb-3 shadow-[0_0_20px_rgba(244,63,94,0.2)]">
                            <ArrowUpFromLine size={32} className="text-rose-400" />
                        </div>
                    ...
                    <div className="space-y-3 pt-2 text-left text-xs">
                        <div className="flex justify-between items-center p-2.5 rounded-xl bg-white/[0.03] border border-white/5">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <button onClick={handleCopyTxId} className="p-1 rounded bg-white/5 hover:bg-white/10 text-rose-400">
                        ...
                        <div className="flex justify-between items-center text-[10px] font-bold uppercase tracking-widest px-1">
                            <span className="text-text-secondary">Network Protocol</span>
                            <span className="text-rose-400 font-black font-mono">{tx.network || 'USDT'}</span>
                        </div>

                        <div className="flex justify-between items-center text-[10px] font-bold uppercase tracking-widest px-1">
                            <span className="text-text-secondary">Type</span>
                            <span className="text-white font-black">Account Payout</span>
                        </div>

                        <div className="flex justify-between items-center text-[10px] font-bold uppercase tracking-widest px-1">
                            <span className="text-text-secondary">Date Initiated</span>
                ...
                <div className="space-y-4">
                    {isLoading ? (
                        ...
                        <div className="glass-card p-12 flex flex-col items-center justify-center text-center animate-slide-up" style={{ animationDelay: '0.05s' }}>
                            ...
                            <p className="text-sm text-text-secondary">Your payout requests will appear here.</p>
                        </div>
                    ) : (
                            ...
                            <div key={tx.id} className="glass-card p-5 space-y-3 animate-slide-up border-white/5" style={{ animationDelay: `${idx * 0.05}s` }}>
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-3">
                                        <div className="w-10 h-10 rounded-xl bg-rose-500/20 flex items-center justify-center shrink-0">
                                            <ArrowUpFromLine size={18} className="text-rose-400" />
                                        </div>
                                        <div>
                                    ...
                                    <div className="text-right shrink-0 ml-3">
                                        <p className="font-black text-rose-400 text-base font-mono tracking-tight">-${tx.amount.toFixed(2)}</p>
                                    </div>
                                </div>
                                ...
                                <div className="grid grid-cols-2 gap-2 pt-1 border-t border-white/5">
                                    ...
                                    <button 
                                        ...
                                        className="flex items-center justify-center gap-1.5 py-2 rounded-xl bg-white/5 border border-white/5 text-[9px] font-black text-rose-400 uppercase tracking-widest hover:bg-rose-500/10 hover:border-rose-500/20 transition-all"
                                    >
                                        <Share2 size={12} />
                                        Voucher Details
                                    </button>
